...
class DeepBeliefNetwork:
	...
	def to_array(self, order='F', dtype=numpy.float32): # order F for gpu
		# , queue, buffer, wait_for=None):
		#weights = numpy.concatenate([numpy.array(x).flatten('F') for x in self.w]).astype(numpy.float32)
		#return cl.enqueue_copy(queue, buffer, weights,wait_for=wait_for)
	...
	def from_array(self, data, order='F', dtype=numpy.float32): # order F for gpu
		#queue, buffer,wait_for=None):
		#items = numpy.concatenate([numpy.zeros_like(x).flatten() for x in self.w]).astype(numpy.float32)
		#e = cl.enqueue_copy(queue, items, buffer,wait_for=wait_for)
