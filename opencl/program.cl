	...
	)
{
	...
	//forward_pass(state_arrays,weight_matrix,rbm_target+1,0);
	...
	)
{
	...
	const size_t weights_offsets = weight_offset(layer);
		...
		//__global const float_t* hidden_state = &state_arrays[hidden_offset];
		//__global const float_t* visible_state = &state_arrays[visible_offset];
...
{
	//forward_pass(state_arrays,weight_matrix,${layer_count}-1,0);
	...
	for (int v = get_group_id(0); v < ${vector_count}; v+=get_num_groups(0))
	{
		...
		for (; x < layer_size( ${layer_count}-1 ); x += get_local_size(0))
		{
			...
			///float_t vv = target[x + v*outsize]-input[x + v*outsize];
			//vv*=vv;
			//val += vv;
		}
